    ...
    try:
        with _get_db_connection() as conn:
            ...
            if removed:
                ...
                smf.printd(f"Clean {len(removed)} binary that is no longer present in the cache.", level="INFO")
            ...
            smf.printd(f"DB sync complete. Total: {len(found_on_disk)} binary, Updated/Inserted:", updated_count, level="INFO")

    except sqlite3.Error as e:
        smf.printd(f"An I/O error occurred during the DB sync transaction.", e, level="ERROR")
...
def call_bin(binary_name: str) -> str:
    ...
    """
    smf.printd(f"Caller makes a path resolution request for", binary_name, level="DEBUG")
    
    try:
        with _get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT path FROM binary_cache WHERE name = ?", (binary_name,))
            row = cursor.fetchone()
            
        if not row:
            smf.printd(f"Resolution failed: Binary not recorded in the database.", binary_name, level="ERROR")
