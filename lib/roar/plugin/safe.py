...
class SafePluginProxy:
    ...
    def __getattr__(self, name):
        try:
            ...
            if callable(attr):
                def safe_method_wrapper(*args, **kwargs):
                    try:
                        ...
                        smf.printd(f"Plugin Runtime Crash [{self._plugin_name}]", f"Method '{name}' failed: {str(e)}", level="ERROR")
            ...
            smf.printd(f"Plugin Attribute Error [{self._plugin_name}]", f"Missing attribute '{name}'", level="WARN")
            return None
        except Exception as e:
...
class NullPlugin:
    ...
    def __getattr__(self, name):
        def silent_absorber(*args, **kwargs):
            smf.printd(f"Dead Call Absorbed [{self._plugin_name}]", f"Attempted to call '{name}' on a failed/unloaded plugin.", level="WARN")
            return None
