...
def read_non_generated(path):
    ...
    return (
        full[0 : generated_begin_index + len(generated_begin)],
...
for name in preferred:
    ...
    if is_single_byte(name):
        ...
        variant = "SingleByte(&data::SINGLE_BYTE_DATA.%s, 0x%04X, %d, %d)" % (
            to_snake_name("iso-8859-8" if name == "ISO-8859-8-I" else name),
            run_bmp_offset,
            run_byte_offset,
            run_length,
        )
...
static_u16_table_from_indexable(
    "BIG5_LEVEL1_HANZI_CODE_POINTS", level1_hanzi_pairs, 0, "big5-hanzi-encode"
)
static_u8_pair_table_from_indexable(
    "BIG5_LEVEL1_HANZI_BYTES", level1_hanzi_pairs, 1, "big5-hanzi-encode"
)
...
for row in xrange(0x7E - 0x20):
    for column in xrange(157):
        ...
        if code_point and code_point >= 0x4E00 and code_point <= 0x9FCB:
            ...
            if (
                code_point == 0x5341
                or code_point == 0x5345
                or not big5_unified_ideograph_bytes[unified_offset]
            ):
...
static_u16_table_from_indexable(
    "JIS0208_LEVEL1_KANJI_CODE_POINTS", level1_kanji_pairs, 0, "kanji-encode"
)
static_u8_pair_table_from_indexable(
    "JIS0208_LEVEL1_KANJI_SHIFT_JIS_BYTES", level1_kanji_pairs, 1, "kanji-encode"
)
...
static_u8_pair_table(
    "KSX1001_UNIFIED_HANJA_BYTES", hanja_unified_bytes, "fast-hanja-encode"
)
static_u8_pair_table(
    "KSX1001_COMPATIBILITY_HANJA_BYTES", hanja_compatibility_bytes, "fast-hanja-encode"
)
...
static_u16_table_from_indexable(
    "GB2312_LEVEL1_HANZI_CODE_POINTS", level1_hanzi_pairs, 0, "gb-hanzi-encode"
)
static_u8_pair_table_from_indexable(
    "GB2312_LEVEL1_HANZI_BYTES", level1_hanzi_pairs, 1, "gb-hanzi-encode"
)
