    ...
    def pre_execute(self, *args, **kwargs) -> dict:
        ...
        for transform_key, decoder_func in self._dispatch_table.items():
            ...
            if transforms.get(transform_key) is True:
                smf.printd(
                    f"[{self.name}] Match found for transform flag",
                    transform_key,
                    level="INFO",
                )
        ...
        if mutated_options:
            smf.printf(
                f"[✓] [{self.name}] Successfully decoded {len(mutated_options)} parameter(s)."
            )
            ...
            return {"handled": False, "modified_options": mutated_options}
